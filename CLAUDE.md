...
**Turning a metafield into an actual storefront filter does not, however, go through the Admin GraphQL API** — searched the full schema for anything filter-related (`FilterOption`, `MetafieldCapability*`, etc.) and found only `adminFilterable` (admin product-list filtering, not storefront) and `smartCollectionCondition` (automated-collection rules, not filters). Confirmed empirically too: after creating the metafield and setting real values on 5 products, the live `/collections/singles` page's facets only showed "Availability" — no Series filter appeared. Enabling a filter is a **Search & Discovery app** setting (Admin → Apps → Search and discovery → Filters), which has no exposed GraphQL mutation. To finish turning this on: open that screen, find "Series / Set" in the list of available filters, and enable it for the Singles collection (or storefront-wide) — a manual, one-time click, not something to attempt via API again.

...
Verified via `shopify theme console --url /collections/singles` + `collection.filters | map: "label"` (more reliable than scraping the rendered page — reads exactly what the theme's Liquid layer sees). As of the storefront-access fix, still showing only `["Availability", "Price"]` — consistent with the indexing-lag pattern hit repeatedly elsewhere in this session (collections, tags, SEO fields all took time to propagate after a correct change); re-check later before assuming something else is wrong.

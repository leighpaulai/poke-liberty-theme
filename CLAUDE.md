...
**Backfilled `custom.series` across the whole catalog** (123 products at the time): 13 already tagged, 19 fictional test-data products deliberately skipped (no real series applies), 88 auto-mapped from the bracketed set code in the product title (regex on `[CODE - ...]` / `[CODE]` / `[CODE:SUBSET - ...]` patterns) against the code table above plus 6 additional values the starter list didn't cover — `Base Set 2`, `Gym Heroes`, `McDonald's Collection`, `Promos` (generic Black Star promos, era-ambiguous by code alone — used for the lone `PR`-coded product), `Scarlet & Violet Promos`, `Sword & Shield Promos`. Choices list is now ~58 values.

**3 products deliberately left unset, not guessed**: "Pikachu Mega Dreams EX" and both "First Partner Illustration Collection" products have no set code anywhere in the title, and nothing else in the data pins them to a specific series confidently. Set these by hand once their actual set is known, or ask Paul.

When bulk-mapping titles to metafield values like this again: extract every distinct bracket code across the catalog first (`products(first: 250) { nodes { title } }`, regex the brackets) and get every code confirmed or flagged before writing anything — several codes here (`PR`, `SVP`, `SWSD`, `M24`) needed a judgment call (promo vs. mainline set, which era) that a naive code→series table would have gotten wrong silently.
